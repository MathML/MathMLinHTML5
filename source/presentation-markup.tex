...
Add italic correction ; how it is preserved when the bounding box does not
change (e.g. mrow) \lxAddClass{issue}

Add top accent attachment how it is preserved when the bounding box does not
change (e.g. mrow) \lxAddClass{issue}

\subsection{Token Elements}

TODO \lxAddClass{issue}
...
The {\tt msub} element is laid out as shown on figure \ref{MsubBoxModel}.
The baseline is the baseline of the base and the subscript is shifted down
by {\tt SubShift}. {\tt SubShift} is initially set to {\tt SubscriptShiftDown}
...
the logical box, a space of width {\tt SpaceAfterScript} is added after
...
\centering
  ...
  \fill[blue!10](0,-1.5) rectangle (7,3.75);
  ...
  (0,0)--(7.5,0);
   ...
   \draw[orange] (4.75, 1) node{LargeOpItalicCorrection};
   \draw[dashed,orange] (4.5,0) -- (4.5,3);

   \draw[->] (6,0) -- (6,1)node[below,rotate=90]{SubTopShift} -- (6,2.25);
...
The {\tt msup} element is laid out as shown on figure \ref{MsupBoxModel}.
...
(section \ref{LaTeX})
{\tt SuperShift} is initially set to {\tt SuperscriptShiftUpCramped} otherwise
it is set to {\tt SuperscriptShiftUp}.
{\tt SuperShift} is increased until the shift for the bottom of the superscript
{\tt SuperBottomShift} is at least {\tt SuperscriptBottomMin}. When determining
the logical box, a space of width {\tt SpaceAfterScript} is added after
...
\centering
  ...
  \fill[blue!10](0,1.5) rectangle (8,-3.75);

  \MathMLBox{0}{0}{1}{1}{red};

  \MathMLBox{5.5}{-3}{.5}{.5}{green};

  \draw[dashed,blue](0,2) rectangle(8.5,-4)
  (0,0)--(8.5,0);
...
The {\tt msubsup} element is laid out as shown on figure \ref{MsubsupBoxModel}.
...
{\tt SuperShift} must be at least {\tt SuperscriptShiftUpCramped} otherwise
it must be at least {\tt SuperscriptShiftUp} (see section \ref{LaTeX}).
...
and the top of the subscript until it is at least {\tt SubSuperscriptGapMin}:
This is first done by continuing to shift the superscript up as long as the
{\tt SupBottomShift} does not exceed {\tt SuperscriptBottomMaxWithSubscript}
...
\centering
  ...
  \fill[blue!10](0,3.75) rectangle (13,-3.75);

  \MathMLBox{0}{0}{1}{1}{red};

  \MathMLBox{5}{-3}{1.5}{.5}{green};

  \draw[dashed,blue](0,4) rectangle(13,-4)
  (0,0)--(13,0);

  \draw[->] (5.5,0) -- (5.5,-1)node[below,rotate=90]{SuperShift} -- (5.5,-3);

  \draw[->] (6.5,0) -- (6.5,-1.75)node[below,rotate=90]{SuperBottomShift} -- (6.5,-2.25);

  \draw[<->] (12.5,-3) -- (12.75,-3)node[left,rotate=90]{SpaceAfterScript} -- (13,-3);

  \MathMLBox{5}{3}{1}{.5}{green};

   \draw[->] (5.75,0) -- (5.75,1)node[below,rotate=90]{SubShift} -- (5.75,3);

  \draw[->] (6.75,0) -- (6.75,1)node[below,rotate=90]{SubTopShift} -- (6.75,2.25);

  \draw[<->] (10,3) -- (10.25,3)node[right,rotate=90]{SpaceAfterScript} -- (10.5,3);

   \draw[<->] (8.5,-2.25) -- (8.5,-2)node[right]{SubSuperGap} -- (8.5,2.25);


\end{tikzpicture}
\label{MsubsupBoxModel}
\end{figure}
...
increments {\tt scriptlevel} by 1 only when accent is "false". Within base, it
...
\item If the base is an operator with {\tt largeop="true"}
  ...
  {\tt UpperLimitBaselineRiseMin} that {\tt UnderGap} is at least
  {\tt LowerLimitGapMin}, that {\tt UnderShift} is at least
  {\tt LowerLimitBaselineDropMin}. {\tt OverExtraAscender} and
...
\item If the base is an operator with {\tt stretchy="true"}
  ...
  {\tt StretchStackGapBelowMin}, that {\tt OverShift} is at least
  {\tt StretchStackTopShiftUp} that {\tt UnderGap} is at least
  {\tt StretchStackGapAboveMin}, that {\tt UnderShift} is at least
  {\tt StretchStackBottomShiftDown}. {\tt OverExtraAscender} and
  {\tt UnderExtraAscender} are zero.
\item In other cases we proceed as follows.
  We first ensure that {\tt UnderGap} is at least  {\tt UnderbarVerticalGap}.
  If overscript is an accent
  and the ink ascent of the base does not exceed {\tt AccentBaseHeight}
  ...
  {\tt OverExtraAscender} are respectively set to {\tt OverbarExtraAscender}
...
that is represented on figure \ref{MmultiscriptsBoxModel}.
\begin{enumerate}
...
In the present specification, we restrict ourselves to a minimal implementation
compatible with HTML tables. First the {\tt mtable} element must be treated as
HTML table and by \cite{MathML3}, {\tt mtable} must accept a displaystyle
attribute with default value {\tt false}. To implement that, one may rely on
the following rules for the user agent stylesheet as
...
Finally, the {\tt mtd} elements must be treated as HTML table cell.
The default value of {\tt rowalign} must be {\tt baseline} and
the default value of {\tt colulmnalign} must be {\tt center}. This can be
achieved with the following CSS:
...
{\tt rowspan} attributes. This can be implemented the same as
...
Suggests some CSS to emulate the default rowspacing, columnspacing,
framespacing? \lxAddClass{issue}
