...
\centering
  ...
  \MathMLBox{5}{3}{.5}{.5}{green};

  \draw[dashed,blue](0,-2) rectangle(8,4)
  (0,0)--(8,0);

   \draw[->] (5.5,0) -- (5.5,1)node[below,rotate=90]{SubShift} -- (5.5,3);

  \draw[->] (6.5,0) -- (6.5,1)node[below,rotate=90]{SubTopShift} -- (6.5,2.25);

  \draw[<->] (7.5,3) -- (7.75,3)node[right,rotate=90]{SpaceAfterScript} -- (8,3);
...
by {\tt SuperShift}. If the {\tt msup} element is cramped then
{\tt SuperShift} is initially set to {\tt SuperscriptShiftUpCramped} otherwise
it is set to {\tt SuperscriptShiftUp} (see section \ref{LaTeX}).
...
\centering
  ...
  \draw[->] (6.5,0) -- (6.5,-1)node[below,rotate=90]{SuperBottomShift} -- (6.5,-2.25);

  \draw[<->] (7.5,-3) -- (7.75,-3)node[left,rotate=90]{SpaceAfterScript} -- (8,-3);
...
If the {\tt msubsup} element is cramped then
...
Interpretation of ``empty'' scripts \lxAddClass{issue}

Discuss horizontal positioning of scripts (italic correction) \lxAddClass{issue}
...
The {\tt displaystyle} changes be achieved with the
following style in the user agent stylesheet (section \ref{UAStylesheet}):
%
\begin{lstlisting}
...
\item Determine the vertical shifts for each subscript/superscript pair
  and use the maximum values for all the scripts.
\item For the horizontal layout of postscripts, use the trailing edge of the
  base or of previous pair as the leading edge of the current pair.
...
the following rules for the user agent stylesheet (section \ref{UAStylesheet}):
%
\begin{lstlisting}
  mtable {
